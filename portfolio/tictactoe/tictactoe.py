...
def minimax(board):
    ...
    best = ()
    alpha = -math.inf   # Alpha is the best possible route so far for MAX player (initialized for the worst)
    beta = math.inf     # Beta is the best possible route so far for MIN player (initialized for the worst)
    if player(board) == X:
        v = -math.inf
        for action in actions(board):
            new_max = max(v, min_value(result(board, action), alpha, beta))
            if new_max > v:
                v = new_max
                best = action
    else:
        v = math.inf
        for action in actions(board):
            new_min = min(v, max_value(result(board, action), alpha, beta))
            if new_min < v:
                v = new_min
                best = action
    return best
